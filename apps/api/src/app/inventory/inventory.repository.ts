...
export class PrismaInventoryRepository implements InventoryRepository {
  ...
  private map(item: any): InventoryItemResponseDto { return { variantId: item.variantId, productName: item.productvariant.product.name, variantLabel: item.productvariant.variantattributevalue.map((link: any) => link.attributevalue.value).join(' / '), quantityOnHand: item.quantityOnHand, quantityReserved: item.quantityReserved, quantityAvailable: item.quantityOnHand - item.quantityReserved, lowStockThreshold: item.lowStockThreshold }; }
  private include = { productvariant: { include: { product: true, variantattributevalue: { include: { attributevalue: true } } } } };
  ...
  async adjustOnHand(variantId: string, quantityChange: number) { const item = await this.prisma.inventoryitem.findFirst({ where: { variantId } }); if (!item) return null; const updated = await this.prisma.$transaction(async (tx) => { const next = await tx.inventoryitem.update({ where: { id: item.id }, data: { quantityOnHand: { increment: quantityChange } }, include: this.include }); await tx.inventorytransaction.create({ data: { id: crypto.randomUUID(), inventoryItemId: item.id, type: quantityChange >= 0 ? 'RESTOCK' : 'ADJUSTMENT', quantityChange } }); return next; }); return this.map(updated); }
  async reserve(variantId: string, quantity: number) { const item = await this.prisma.inventoryitem.findFirst({ where: { variantId } }); if (!item) return null; const updated = await this.prisma.$transaction(async (tx) => { const next = await tx.inventoryitem.update({ where: { id: item.id }, data: { quantityReserved: { increment: quantity } }, include: this.include }); await tx.inventorytransaction.create({ data: { id: crypto.randomUUID(), inventoryItemId: item.id, type: 'RESERVATION', quantityChange: -quantity } }); return next; }); return this.map(updated); }
  async createItem(item: { variantId: string; productName: string; variantLabel: string; quantityOnHand: number; lowStockThreshold?: number }) { const variant = await this.prisma.productvariant.findUniqueOrThrow({ where: { id: item.variantId }, include: { product: true } }); const warehouse = await this.prisma.warehouse.findFirstOrThrow({ where: { sellerId: variant.sellerId, isActive: true } }); const created = await this.prisma.inventoryitem.upsert({ where: { variantId_warehouseId: { variantId: item.variantId, warehouseId: warehouse.id } }, update: { quantityOnHand: item.quantityOnHand, lowStockThreshold: item.lowStockThreshold ?? 5 }, create: { id: crypto.randomUUID(), variantId: item.variantId, sellerId: variant.sellerId, warehouseId: warehouse.id, quantityOnHand: item.quantityOnHand, lowStockThreshold: item.lowStockThreshold ?? 5 } as any, include: this.include }); return this.map(created); }
}
