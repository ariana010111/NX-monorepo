...
export class PrismaOrdersRepository implements OrdersRepository {
  ...
  private include = { orderitem: { include: { productvariant: { include: { variantattributevalue: { include: { attributevalue: true } }, productimage: { where: { isPrimary: true }, take: 1 } } }, product: true } } };
  private map(order: any): OrderResponseDto { return { id: order.id, orderNumber: order.orderNumber, email: order.email, userId: order.userId, status: order.status, currency: order.currency, subtotal: Number(order.subtotal), discountTotal: Number(order.discountTotal) || undefined, couponCode: order.couponCode ?? undefined, grandTotal: Number(order.grandTotal), items: order.orderitem.map((item: any) => ({ variantId: item.variantId, productName: item.productNameSnapshot, variantLabel: item.variantAttributesSnapshot ? Object.values(item.variantAttributesSnapshot as Record<string, string>).join(' / ') : item.variantSkuSnapshot, unitPrice: Number(item.unitPriceSnapshot), quantity: item.quantity, lineTotal: Number(item.lineTotal) })), placedAt: order.placedAt.toISOString() }; }
  async create({ dto, userId, discountTotal, couponCode }: CreateOrderInput) {
    const variants = await this.prisma.productvariant.findMany({ where: { id: { in: dto.items.map((item) => item.variantId) }, deletedAt: null }, include: { product: true, seller: true, variantattributevalue: { include: { attributevalue: { include: { attribute: true } } } }, productimage: { where: { isPrimary: true }, take: 1 } } });
    const byId = new Map(variants.map((variant) => [variant.id, variant]));
    const items = dto.items.map((input) => { const variant = byId.get(input.variantId); if (!variant) throw new Error(`Variant "${input.variantId}" not found`); const attributes = Object.fromEntries(variant.variantattributevalue.map((link: any) => [link.attributevalue.attribute.name, link.attributevalue.value])); return { input, variant, attributes, lineTotal: Math.round(input.unitPrice * input.quantity * 100) / 100 }; });
    ...
    const order = await this.prisma.order.create({ data: { id: crypto.randomUUID(), orderNumber: `ORD-${new Date().getFullYear()}-${Date.now()}`, email: dto.email, userId, status: 'PENDING_PAYMENT', subtotal, discountTotal, grandTotal, couponCode, shippingAddress, billingAddress: shippingAddress, updatedAt: new Date(), orderitem: { create: items.map(({ input, variant, attributes, lineTotal }) => ({ id: crypto.randomUUID(), sellerId: variant.sellerId, productId: variant.productId, variantId: variant.id, productNameSnapshot: variant.product.name, variantSkuSnapshot: variant.sku, variantAttributesSnapshot: attributes, imageUrlSnapshot: variant.productimage[0]?.url, unitPriceSnapshot: input.unitPrice, quantity: input.quantity, lineTotal, updatedAt: new Date() })) } } as any, include: this.include });
  ...
  async updateStatus(id: string, status: string) { const result = await this.prisma.order.updateMany({ where: { id }, data: { status: status as any } }); if (!result.count) return null; await this.prisma.orderstatushistory.create({ data: { id: crypto.randomUUID(), orderId: id, status } }); return this.map(await this.prisma.order.findUniqueOrThrow({ where: { id }, include: this.include })); }
}
