...
export class PrismaRefreshTokensRepository implements RefreshTokensRepository {
  ...
  async create(userId: string) { const token = randomBytes(32).toString('hex'); await this.prisma.refreshtoken.create({ data: { id: randomUUID(), token, userId, expiresAt: new Date(Date.now() + REFRESH_TOKEN_TTL_MS) } }); return token; }
